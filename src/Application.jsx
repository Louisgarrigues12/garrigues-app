...
// Fusion des tarifs : une valeur vide venant de la base n'efface jamais un prix par défaut.
const mergeT=(base,t)=>{const m={...base};for(const k in t){const v=t[k];if(v!==""&&v!=null)m[k]=v;}return m;};
...
export default function App(){
  ...
  const SHELL=vw>=760?Math.min(vw-48,1180):430;   // pages lecture & formulaires
  const WIDE=vw>=760?Math.min(vw-48,1500):430;    // pages listes & grilles
  ...
  useEffect(()=>{
    ...
    document.body.style.background=C.light;
  ...
  if(page==="splash")return(
    <div style={{fontFamily:"system-ui,sans-serif",width:"100%",minHeight:"100vh",background:`linear-gradient(160deg,${C.navy},#0d2347)`,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{textAlign:"center",color:C.white}}><BN s={44}/><p style={{opacity:.5,fontSize:13,marginTop:8}}>Chargement…</p></div>
    </div>
  );

  if(!uid)return(
    <div style={{fontFamily:"system-ui,sans-serif",width:"100%",minHeight:"100vh",background:`linear-gradient(160deg,${C.navy},#0d2347)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:28}}>
  ...
  if(page==="fiche"&&fiche){
    ...
    return(
      <div style={{fontFamily:"system-ui,sans-serif",width:"100%",minHeight:"100vh",background:C.light,display:"flex",flexDirection:"column"}}>
  ...
  if(page==="home")return(
    <div style={{fontFamily:"system-ui,sans-serif",width:"100%",minHeight:"100vh",background:`linear-gradient(160deg,${C.navy},#0d2347 60%,#111827)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:28}}>
      <BN s={42}/>
      ...
      {[["🥔  Catalogue & commande","catalogue"],["📦  Mes commandes","orders"],["💬  Conseiller IA","chat"],["⚙️  Administration","admin"]].map(([l,p])=>(
        <button key={p} onClick={()=>setPage(p)} style={{width:"100%",maxWidth:460,padding:"15px 20px",borderRadius:14,border:"1.5px solid rgba(255,255,255,.15)",background:"rgba(255,255,255,.06)",color:C.white,fontSize:15,fontWeight:700,cursor:"pointer",marginBottom:10,textAlign:"left"}}>
  ...
  if(page==="chat")return(
    <div style={{fontFamily:"system-ui,sans-serif",width:"100%",height:"100vh",display:"flex",flexDirection:"column",background:C.light}}>
      <Hdr title="Conseiller IA" onBack={()=>setPage("home")}/>
      <div style={{flex:1,overflowY:"auto",padding:"12px 12px 6px",width:"100%",maxWidth:860,margin:"0 auto",boxSizing:"border-box"}}>
        {msgs.map((m,i)=>(
      ...
      <div style={{padding:"8px 12px 12px",background:C.white,borderTop:`1px solid ${C.lgray}`,flexShrink:0,display:"flex",gap:8,width:"100%",maxWidth:860,margin:"0 auto",boxSizing:"border-box"}}>
  ...
  // CATALOGUE (default)
  return(
    <div style={{fontFamily:"system-ui,sans-serif",width:"100%",minHeight:"100vh",background:C.light}}>
      ...
      <div style={{padding:"9px 11px"}}>
        ...
        {filtered.map(v=>{
          ...
          const fps=[];v.cal.forEach(c=>cs.forEach((_,i)=>{const p=parseFloat(tarifs[`${v.id}__${c}__${i}`]);if(!isNaN(p))fps.push(p);}));
          const fp=fps.length?Math.min(...fps):null;
            ...
            <div key={v.id}
              ...
              <div style={{display:"flex",alignItems:"center",gap:11,pointerEvents:"none"}}>
                ...
                <div style={{textAlign:"right",flexShrink:0}}>
                  <div style={{fontSize:11,color:fp!=null?m.c:C.gray,fontWeight:fp!=null?700:400}}>{fp!=null?`À partir de ${fp.toFixed(2)} €`:"Voir tarifs"}</div>
                </div>
              </div>
            </div>
          );
